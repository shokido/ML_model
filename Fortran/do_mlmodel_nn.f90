program do_mlmodel_nn
  ...
  real(idx),allocatable :: qq_1d(:),l_1d(:)
  real(idx),allocatable :: qq_next_1d(:),l_next_1d(:)
  ...
  allocate(qq_1d(1:(nz_q)));allocate(l_1d(1:(nz_q)))
  allocate(qq_next_1d(1:(nz_q)));allocate(l_next_1d(1:(nz_q)))
  ...
  do itime = 1,ntime
     ...
     !sflx=0.0_idx
     !uflx=0.1;vflx=0.0
     !hflx_nosolar=-70*sin(2.0*pi*(dt*itime/(60*60*24)/5))
     !uflx=0.2*sin(2.0*pi*(dt*itime/(60*60*24)/5))

     !     hflx_nosolar=0_idx
!     hflx_solar=0
!     hflx_nosolar=-50_idx
!     hflx_solar=-1*hflx_nosolar*(1+sin(2.0*dt*itime*pi/(24.0*60.0*60.0)))
!     hflx_solar=200*(1+sin(2.0*dt*itime*pi/(24.0*60.0*60.0*365)))
!     hflx_solar=200*(1+sin(2.0*dt*itime*pi/(24.0*60.0*60.0)))
     ...
     call cal_next_theta(dt,nz_rho,z_rho,z_q,temp_1d,kt_1d,penet_1d,&
          & hflx_nosolar,temp_correct_1d,temp_next_1d)
     call cal_next_salt(dt,nz_rho,z_rho,z_q,salt_1d,&
          & ks_1d,sflx,salt_correct_1d,salt_next_1d)
     call cal_next_uv(dt,nz_rho,z_rho,z_q,u_1d,v_1d,&
          & km_1d,coriolis,uflx,vflx,u_correct_1d,v_correct_1d,&
          & u_next_1d,v_next_1d)
     call cal_next_qq(dt,nz_rho,z_rho,z_q,qq_1d,bvf_1d,shear_1d, &
  ...
  deallocate(penet_1d)
  deallocate(qq_1d);deallocate(qq_next_1d)
